    ...
    <div className='w-full min-h-screen bg-black'>
      <div className='w-full h-[80px] flex justify-between items-center px-[30px] text-white'>
        <div onClick={() => navigate("/")}>
          <IoArrowBack className='text-white cursor-pointer w-[25px] h-[25px]' /></div>
        <div className='font-semibold text-[20px]'>
          {profileData?.userName || "Unknown User"}</div>
      ...
      <div className='w-full h-[100px] flex items-center justify-center gap-[40px] md:gap-[60px] px-[20%] pt-[30px] text-white'>
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center justify-center gap-[8px] relative">
            <div className="relative flex" style={{ width: "60px", height: "40px" }}>
              {profileData?.followers?.slice(0, 3).map((user, index) => (
                <div key={index} className="w-[35px] h-[35px] border-2 border-black rounded-full overflow-hidden absolute"
          ...
          <div className="text-[18px] md:text-[22px] text-[#ffffffc7] mt-[2px]">
            Followers
          </div>
        </div>
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center justify-center gap-[10px] relative">
            <div className="relative flex" style={{ width: "70px", height: "40px" }}>
              {profileData?.following?.slice(0, 3).map((user, index) => (
                <div key={index} className="w-[35px] h-[35px] border-2 border-black rounded-full overflow-hidden absolute"
                  style={{left: `${index * 13}px`, zIndex: 10 - index,}}>
                  <img src={user?.profileImage || dp} alt="" className="w-full h-full object-cover"/>
                </div>
              ))}
            </div>
          ...
          {profileData?._id == userData._id &&
            <div className='w-[90%] max-w-[500px] h-[80px] bg-[white] rounded-full flex justify-center items-center gap-[10px]'>
              <div className={`${postType == "posts" ? "bg-black text-white shadow-2xl shadow-black" : ""} w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold
                hover:bg-black rounded-full hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`}
