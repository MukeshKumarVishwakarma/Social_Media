...
function LeftHome() {
  ...
  console.log("LeftHome UserData" ,userData);
    ...
    ${showNotification ? "overflow-hidden" : "overflow-auto" }`}>
      <div className='w-full h-[100px] flex items-center justify-between p-[20px]'>
        ...
        <div className='relative z-[100]' onClick={()=>setShowNotification(prev=>!prev)}>
          ...
          (<div className='w-[10px] h-[10px] bg-blue-600 rounded-full absolute top-0 right-[-5px]'>
          </div> )}
      ...
      <div className='flex items-center w-full justify-between gap-[10px] px-[10px] border-b-2 border-b-gray-900 py-[10px]'>
        <div className='flex items-center gap-[10px]'>
          ...
          <div>
            <div className='text-[18px] text-white font-semibold'>{userData?.userName || "userName"}</div>
            <div className='text-[15px] text-gray-400 font-semibold'>{userData?.name || "name"}</div>
          </div>
        </div>
        <div className='text-blue-500 cursor-pointer font-semibold' onClick={handleLogOut}>Log Out</div>
      </div>
